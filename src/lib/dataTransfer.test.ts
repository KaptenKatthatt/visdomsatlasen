...
const titleFor = (type: Origin, id: string): string | undefined => `${type}:${id}`
...
describe('tillExport', () => {
  it('bygger en exportpost med format, version och uppslagna titlar', () => {
    const exporten = toExport(collections(), titleFor, '2026-07-14T10:00:00.000Z')
...
describe('lasImport', () => {
  ...
  it('importerar en äldre export med svenska nycklar och värden utan förlust', () => {
    ...
    expect(out?.savedRooms[0]).toMatchObject({ id: 'rum-a', savedWhen: '2026-06-03T00:00:00.000Z' })
    expect(out?.savedPaths[0]).toMatchObject({ id: 'vandring-x', savedWhen: null })
    expect(out?.bookmarks.topics).toEqual(['topic-1'])
  })
})
...
describe('mergaImport', () => {
  it('unionar sparade poster och bokmärken utan att röra befintlig data', () => {
    ...
    const incoming = toExport(collections(), titleFor, '2026-07-14T10:00:00.000Z')
    const out = mergeImport(current, incoming)
    expect(Object.keys(out.savedRooms).sort()).toEqual(['rum-a', 'rum-b'])
    expect(out.bookmarks).toMatchObject({ egen: true, 'topic-1': true })
    expect(Object.keys(out.chapterBookmarks)).toEqual(['w/b:3'])
  })
  ...
  it('låter den nyast uppdaterade anteckningen vinna vid konflikt', () => {
    ...
    const out = mergeImport(older, incoming)
    expect(out.notes['rum-a']?.text).toBe('en tanke')

    const newer: PersonalCollections = {
      ...older,
      notes: {
        'rum-a': { originType: 'room', originId: 'rum-a', text: 'nyare', created: '2026-07-01T00:00:00.000Z', updated: '2026-07-20T00:00:00.000Z' },
      },
    }
    expect(mergeImport(newer, incoming).notes['rum-a']?.text).toBe('nyare')
  })
})

describe('tillMarkdown', () => {
  it('speglar anteckningar och sparat i läsbar Markdown', () => {
    const md = toMarkdown(toExport(collections(), titleFor, '2026-07-14T10:00:00.000Z'))
