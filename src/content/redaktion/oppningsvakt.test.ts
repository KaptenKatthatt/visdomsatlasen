...
// Var och en avslutade en öppning och lockade vidare utan att berätta vad.
...
describe('ärTeaseröppning', () => {
  it('flaggar varje borttagen teaser-rad', () => {
    for (const öppning of BORTTAGNA_TEASERS) {
      expect(ärTeaseröppning(öppning), öppning).toBe(true)
    }
  })

  it('släpper igenom öppningar som landar i vardagen eller en öppen fråga', () => {
    for (const öppning of GODA_ÖPPNINGAR) {
      expect(ärTeaseröppning(öppning), öppning).toBe(false)
    }
  })
